...
func (mux *ServeMux) ProcessTask(ctx context.Context, task *asynq.Task) error {
...
func (mux *ServeMux) Handler(t *asynq.Task) (h []asynq.Handler, pattern string) {
	mux.mu.RLock()
...
func (mux *ServeMux) match(typename string) (h []asynq.Handler, pattern string) {
...
func (mux *ServeMux) Handle(pattern string, handlers []asynq.Handler) {
	mux.mu.Lock()
	defer mux.mu.Unlock()
	...
	e := muxEntry{h: handlers, pattern: pattern}
	mux.m[pattern] = e
...
func (mux *ServeMux) HandleFunc(pattern string, handlers []func(context.Context, *asynq.Task) error) {
	if handlers == nil || len(handlers) == 0 {
		panic("asynq: nil handler")
	}
	var asynqHandlers []asynq.Handler
	for i, fn := range handlers {
		asynqHandlers[i] = asynq.HandlerFunc(fn)
	}
	mux.Handle(pattern, asynqHandlers)
}
...
func NotFound(_ context.Context, task *asynq.Task) error {
	return fmt.Errorf("handler not found for task %q", task.Type())
}

// NotFoundHandler returns a simple task handler that returns a “not found“ error.
func NotFoundHandler() []asynq.Handler {
	return []asynq.Handler{asynq.HandlerFunc(NotFound)}
}
